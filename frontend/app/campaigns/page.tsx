...
const TECHNIQUES = [
  ...
  { id: 'deauth',    label: 'Deauth',    desc: 'Ataque de desautenticaciÃ³n' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card space-y-4">
          <p className="section-title flex items-center gap-2">
            <Target className="w-3.5 h-3.5" /> CampaÃ±as
          </p>

          <div className="space-y-2">
            <input className="input text-xs" placeholder="Nombre de campaÃ±a" value={newName}
              onChange={e => setNewName(e.target.value)} />
            <input className="input text-xs" placeholder="DescripciÃ³n (opcional)" value={newDesc}
              onChange={e => setNewDesc(e.target.value)} />
            <button className="btn-primary w-full text-xs" onClick={handleCreate} disabled={!newName}>
              <Plus className="w-3.5 h-3.5" /> Nueva campaÃ±a
            </button>
          </div>
          ...
          <div className="space-y-1 max-h-[400px] overflow-y-auto">
            {campaigns.length === 0 ? (
              <p className="text-muted text-xs text-center py-4">Sin campaÃ±as</p>
            ) : campaigns.map(c => (
              <div key={c.id}
        ...
        <div className="lg:col-span-2 space-y-4">
          {selected ? (
            <>
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                  <p className="section-title mb-0">Objetivos Â· {selected.name}</p>
                  <div className="flex gap-2">
                    ...
                    >
                      {auditing
                        ...
                        : <><Play className="w-3.5 h-3.5" /> Iniciar auditorÃ­a</>}
                    </button>
                    <button
                ...
                {targets.length === 0 ? (
                  <p className="text-muted text-xs text-center py-4">Sin objetivos. AÃ±ade redes detectadas.</p>
                ) : (
                  <div className="space-y-2">
                    {targets.map(t => {
                      ...
                      return (
                        <div key={t.id} className="rounded-lg bg-surface border border-border/30 p-3">
                          <div className="flex items-center gap-3 text-xs font-mono mb-2">
                            <span className="text-accent">{t.bssid ?? 'â€”'}</span>
              ...
              {/* Add from scanner */}
              <div className="card">
                <p className="section-title">AÃ±adir desde escÃ¡ner</p>
                <div className="space-y-1 max-h-[220px] overflow-y-auto">
                  {networks.length === 0 ? (
                    <p className="text-muted text-center py-3 text-xs">Ejecuta el escÃ¡ner primero</p>
                  ) : networks.map(net => (
                      ...
                      <div key={r.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-surface border border-border/30 text-xs">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-muted">
                            {new Date(r.created_date).toLocaleString('es')} Â· {(r.size / 1024).toFixed(1)} KB
                          </p>
                        </div>
            ...
            <div className="card flex flex-col items-center justify-center h-64 text-muted text-sm gap-3">
              ...
              <p>Selecciona una campaÃ±a para gestionar sus objetivos</p>
            </div>
          )}
        </div>
      </div>
